/**
 * RegistryPersister.ts
 * Description: Startup runner scheduler interface
 * Part of CorvusX: Intelligence in the Shadows
 * Generated on: 2026-05-31T17:59:24.088Z
 */
...
export interface RegistryPersisterConfig {
  enabled: boolean;
  debugMode?: boolean;
  maxRetries: number;
  timeoutMs: number;
...
export interface RegistryPersisterState {
  initialized: boolean;
  lastUpdated: number;
...
export class RegistryPersisterManager extends EventEmitter {
  private config: RegistryPersisterConfig;
  private state: RegistryPersisterState;
  private intervalId: NodeJS.Timeout | null = null;
  private historyLog: string[] = [];

  constructor(config: RegistryPersisterConfig) {
    super();
    this.config = config;
    this.state = {
      initialized: false,
  ...
  public updateConfig(newConfig: Partial<RegistryPersisterConfig>): void {
    this.config = { ...this.config, ...newConfig };
    this.log('Configuration updated.');
    this.emit('config_updated', this.config);
  }

  public getState(): RegistryPersisterState {
    return { ...this.state };
  }

  private log(msg: string): void {
    const formatted = `[${new Date().toISOString()}] [RegistryPersister] ${msg}`;
