 ...
 * Description: Packet sniffer and protocol parser
 * Part of CorvusX: Intelligence in the Shadows
 * Generated on: 2026-05-31T17:59:22.159Z
 */
...
export interface NetworkInterceptorState {
  initialized: boolean;
  lastUpdated: number;
...
export class NetworkInterceptorManager extends EventEmitter {
  private config: NetworkInterceptorConfig;
  private state: NetworkInterceptorState;
  ...
  public updateConfig(newConfig: Partial<NetworkInterceptorConfig>): void {
    this.config = { ...this.config, ...newConfig };
    this.log('Configuration updated.');
    this.emit('config_updated', this.config);
  }

  public getState(): NetworkInterceptorState {
    return { ...this.state };
  }

  private log(msg: string): void {
    const formatted = `[${new Date().toISOString()}] [NetworkInterceptor] ${msg}`;
