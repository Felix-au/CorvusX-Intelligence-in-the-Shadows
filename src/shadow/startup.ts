/**
 * StartupManager.ts
 * Description: Daemon autorun scheduler config loader
 * Part of CorvusX: Intelligence in the Shadows
 * Generated on: 2026-05-31T17:59:27.249Z
 */
...
export interface StartupManagerState {
  initialized: boolean;
  lastUpdated: number;
...
export class StartupManagerManager extends EventEmitter {
  private config: StartupManagerConfig;
  private state: StartupManagerState;
  private intervalId: NodeJS.Timeout | null = null;
  private historyLog: string[] = [];

  constructor(config: StartupManagerConfig) {
    super();
    this.config = config;
    this.state = {
      initialized: false,
  ...
  public updateConfig(newConfig: Partial<StartupManagerConfig>): void {
    this.config = { ...this.config, ...newConfig };
    this.log('Configuration updated.');
    this.emit('config_updated', this.config);
  }

  public getState(): StartupManagerState {
    return { ...this.state };
  }

  private log(msg: string): void {
    const formatted = `[${new Date().toISOString()}] [StartupManager] ${msg}`;
