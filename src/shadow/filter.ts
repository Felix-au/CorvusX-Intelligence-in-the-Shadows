/**
 * FilterRules.ts
 * Description: IP ranges and ports whitelister rules engine
 * Part of CorvusX: Intelligence in the Shadows
 * Generated on: 2026-05-31T17:59:26.859Z
 */
...
export class FilterRulesManager extends EventEmitter {
  private config: FilterRulesConfig;
  private state: FilterRulesState;
  private intervalId: NodeJS.Timeout | null = null;
  private historyLog: string[] = [];

  constructor(config: FilterRulesConfig) {
    super();
    this.config = config;
    this.state = {
      initialized: false,
  ...
  public updateConfig(newConfig: Partial<FilterRulesConfig>): void {
    this.config = { ...this.config, ...newConfig };
    this.log('Configuration updated.');
    this.emit('config_updated', this.config);
  }

  public getState(): FilterRulesState {
    return { ...this.state };
  }

  private log(msg: string): void {
    const formatted = `[${new Date().toISOString()}] [FilterRules] ${msg}`;
