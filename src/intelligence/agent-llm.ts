...
export interface AgentLLMState {
  initialized: boolean;
  lastUpdated: number;
...
export class AgentLLMManager extends EventEmitter {
  ...
  public updateConfig(newConfig: Partial<AgentLLMConfig>): void {
    this.config = { ...this.config, ...newConfig };
    this.log('Configuration updated.');
    this.emit('config_updated', this.config);
  }

  public getState(): AgentLLMState {
    return { ...this.state };
  }

  private log(msg: string): void {
    const formatted = `[${new Date().toISOString()}] [AgentLLM] ${msg}`;
