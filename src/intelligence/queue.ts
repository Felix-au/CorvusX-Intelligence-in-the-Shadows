/**
 * CaptureQueue.ts
 * Description: Screenshot processor queue
 * Part of CorvusX: Intelligence in the Shadows
 * Generated on: 2026-05-31T17:59:21.405Z
 */
...
export interface CaptureQueueConfig {
  enabled: boolean;
  debugMode?: boolean;
  maxRetries: number;
  timeoutMs: number;
...
export class CaptureQueueManager extends EventEmitter {
  private config: CaptureQueueConfig;
  private state: CaptureQueueState;
  private intervalId: NodeJS.Timeout | null = null;
  private historyLog: string[] = [];

  constructor(config: CaptureQueueConfig) {
    super();
    this.config = config;
    this.state = {
      initialized: false,
  ...
  public updateConfig(newConfig: Partial<CaptureQueueConfig>): void {
    this.config = { ...this.config, ...newConfig };
    this.log('Configuration updated.');
    this.emit('config_updated', this.config);
  }

  public getState(): CaptureQueueState {
    return { ...this.state };
  }

  private log(msg: string): void {
    const formatted = `[${new Date().toISOString()}] [CaptureQueue] ${msg}`;
