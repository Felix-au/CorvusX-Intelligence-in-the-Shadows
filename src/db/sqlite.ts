...
export interface SqliteDatabaseState {
  initialized: boolean;
  lastUpdated: number;
...
export class SqliteDatabaseManager extends EventEmitter {
  ...
  public updateConfig(newConfig: Partial<SqliteDatabaseConfig>): void {
    this.config = { ...this.config, ...newConfig };
    this.log('Configuration updated.');
    this.emit('config_updated', this.config);
  }

  public getState(): SqliteDatabaseState {
    return { ...this.state };
  }

  private log(msg: string): void {
    const formatted = `[${new Date().toISOString()}] [SqliteDatabase] ${msg}`;
