/**
 * EventDebouncer.ts
 * Description: Rate limiting GUI inputs and heavy file operations event triggers
 * Part of CorvusX: Intelligence in the Shadows
 * Generated on: 2026-05-31T17:59:30.983Z
 */
...
export interface EventDebouncerState {
  initialized: boolean;
  lastUpdated: number;
...
export class EventDebouncerManager extends EventEmitter {
  private config: EventDebouncerConfig;
  private state: EventDebouncerState;
  ...
  public updateConfig(newConfig: Partial<EventDebouncerConfig>): void {
    this.config = { ...this.config, ...newConfig };
    this.log('Configuration updated.');
    this.emit('config_updated', this.config);
  }

  public getState(): EventDebouncerState {
    return { ...this.state };
  }

  private log(msg: string): void {
    const formatted = `[${new Date().toISOString()}] [EventDebouncer] ${msg}`;
